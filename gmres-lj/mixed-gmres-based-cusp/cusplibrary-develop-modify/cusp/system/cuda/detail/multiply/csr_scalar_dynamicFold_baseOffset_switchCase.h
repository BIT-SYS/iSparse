...
{ 
	...
	// __shared__ ValueType sum[16 * BLOCK_SIZE]; 
	ValueType sum[16]; 
	for  (IndexType i  = 0; i < 16; i++) sum[i] = 0.0;
    ...
    for(IndexType row = thread_id; row < num_rows; row += grid_size)
    {
        ...
        for (IndexType jj = s_rowPtr_col; jj < e_rowPtr_col; jj++) {
			...
			if (sign == 0) { // base
				...
				// printf("1: thread %u: sum[%u] (%.6f) += Ax[%u] * x[%u] = %.6f * %.6f\n", thread_id, blockId, sum[blockId], s_rowPtr_val, colIdx, Ax[s_rowPtr_val], x[colIdx]);
				switch(blockId)  {
					case 0: sum[0] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 1: sum[1] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 2: sum[2] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 3: sum[3] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 4: sum[4] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 5: sum[5] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 6: sum[6] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 7: sum[7] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 8: sum[8] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 9: sum[9] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 10: sum[10] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 11: sum[11] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 12: sum[12] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 13: sum[13] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 14: sum[14] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					case 15: sum[15] += Ax[s_rowPtr_val++] * x[colIdx]; break;
					default: break;
				}
				...
				for (IndexType k = 0; k < sign; k++) { 
					...
					//if (thread_id == 0)
					//	printf("thread_id: %u, blockId: %u row: %u jj: %u colIdx: %u\n", thread_id, blockId, row, jj, colIdx);
		...
		for (IndexType i = 0; i < e_rowIdx - s_rowIdx; i++) {
			// printf("3:thread_id %u: y[%u + %u](%.6f) = sum[%u] = %.6f\n", thread_id, s_rowIdx, i, y[s_rowIdx+i], i, sum[i]);
         	y[s_rowIdx + i] = sum[i];
			sum[i] = 0.0;
		}
//		for (IndexType i = 0; i < e_rowIdx - s_rowIdx; i++) {
