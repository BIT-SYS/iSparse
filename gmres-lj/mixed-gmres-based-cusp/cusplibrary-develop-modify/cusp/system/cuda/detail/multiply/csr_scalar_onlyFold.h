...
{ 
    ...
    {
        const IndexType s_rowPtr = Ap[row];
        const IndexType e_rowPtr = Ap[row+1];
        ...
        for (IndexType jj = s_rowPtr; jj < e_rowPtr; jj++) {
			...
			colIdx = (curColIdx >> SHIFT_BITS );
			sum[blockId  * blockDim.x + threadIdx.x] += Ax[jj] * x[colIdx];
        }

		IndexType tmp = row << SHIFT_BITS;
#pragma unroll
		for (IndexType i = 0; i < FOLDGRAIN; i++) {
			IndexType tmp_sum_index = i * BLOCK_SIZE + threadIdx.x;
         	y[i + tmp] = sum[tmp_sum_index];
			sum[tmp_sum_index] = 0;
		}
    }
}
                     ...
                     array1d_format)
{
    ...
    // const size_t block_row = (A.num_rows + FOLDGRAIN - 1) / FOLDGRAIN;
