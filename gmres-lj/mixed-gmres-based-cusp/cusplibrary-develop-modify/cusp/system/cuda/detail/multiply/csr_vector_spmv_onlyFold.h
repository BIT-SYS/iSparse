                       ...
                       BinaryFunction2 reduce)
{
    ...
    typedef typename thrust::iterator_value<ValueIterator1>::type ValueType;
     ...
     __shared__ volatile ValueType sdata[FOLD][VECTORS_PER_BLOCK * THREADS_PER_VECTOR + THREADS_PER_VECTOR / 2];  // padded to avoid reduction conditionals
    // __shared__ volatile ValueType sdata[MAX_FOLD_ROWS][VECTORS_PER_BLOCK * THREADS_PER_VECTOR + 32];  // padded to avoid reduction conditionals
    ...
    for(IndexType row = vector_id; row < num_rows; row += num_vectors)
    {
        ...
        const IndexType rowPtr_start = ptrs[vector_lane][0];                   //same as: rowPtr_start = Ap[row];
        const IndexType rowPtr_stop = ptrs[vector_lane][1];                   //same as: rowPtr_start = Ap[row];

		for (IndexType i = 0; i < FOLD; i++) {
			sdata[i][threadIdx.x] = (ValueType)0.0;
		}

		ValueType temp;

        if (THREADS_PER_VECTOR == 32 && rowPtr_stop - rowPtr_start > 32)
        {
            // ensure aligned memory access to Aj and Ax

            IndexType jj = rowPtr_start - (rowPtr_start & (THREADS_PER_VECTOR - 1)) + thread_lane;

            // accumulate local sums
            if(jj >= rowPtr_start && jj < rowPtr_stop) {
				IndexType curColIdx = Aj[jj];
				IndexType blockId = (curColIdx & FOLD_MASK);
				IndexType colIdx = (curColIdx >> FOLD_BITS);
				temp = sdata[blockId][threadIdx.x];
				sdata[blockId][threadIdx.x] = temp = reduce(temp, combine(Ax[jj], x[colIdx]));
			}

            // accumulate local sums
            for(jj += THREADS_PER_VECTOR; jj < rowPtr_stop; jj += THREADS_PER_VECTOR) {
				IndexType curColIdx = Aj[jj];
				IndexType blockId = (curColIdx & FOLD_MASK);
				IndexType colIdx = (curColIdx >> FOLD_BITS);
				temp = sdata[blockId][threadIdx.x];
        ...
        {
            // accumulate local sums
            for(IndexType jj = rowPtr_start + thread_lane; jj < rowPtr_stop; jj += THREADS_PER_VECTOR) {
				IndexType curColIdx = Aj[jj];
				IndexType blockId = (curColIdx & FOLD_MASK);
				IndexType colIdx = (curColIdx >> FOLD_BITS);
				temp = sdata[blockId][threadIdx.x];
		...
		IndexType rowIdx = row * FOLD + thread_lane;
        // if (thread_lane == 0)  {
        ...
        //     	y[row + rowIdx_start] = ValueType(sdata[row][threadIdx.x]);
  		...
  		// }
		if (thread_lane < FOLD)
			y[rowIdx] = ValueType(sdata[thread_lane][threadIdx.x - thread_lane]);
    }
}
