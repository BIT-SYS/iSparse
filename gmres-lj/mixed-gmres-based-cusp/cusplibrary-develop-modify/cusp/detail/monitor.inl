...
namespace cusp
{
    ...
    monitor<ValueType>::monitor(const VectorType &b, size_t iteration_limit, Real relative_tolerance, Real absolute_tolerance, bool verbose)
    ...
    template <typename ValueType>
    void
    monitor<ValueType>::operator++(void)
    {
        ++iteration_count_;
    }
    ...
    template <typename ValueType>
    bool
    monitor<ValueType>::converged(void) const
    {
    ...
    monitor<ValueType>::residual_norm(void) const
    {
        return r_norm;
    }
    ...
    template <typename ValueType>
    size_t
    monitor<ValueType>::iteration_count(void) const
    {
    ...
    template <typename ValueType>
    size_t
    monitor<ValueType>::iteration_limit(void) const
    {
    ...
    template <typename ValueType>
    typename monitor<ValueType>::Real
    monitor<ValueType>::relative_tolerance(void) const
    {
    ...
    template <typename ValueType>
    typename monitor<ValueType>::Real
    monitor<ValueType>::absolute_tolerance(void) const
    {
    ...
    template <typename ValueType>
    typename monitor<ValueType>::Real
    monitor<ValueType>::tolerance(void) const
    {
    ...
    template <typename ValueType>
    void
    monitor<ValueType>::set_verbose(bool verbose_)
    {
        verbose = verbose_;
    }
    ...
    template <typename ValueType>
    bool
    monitor<ValueType>::is_verbose(void)
    {
        return verbose;
    }
    ...
    void
    monitor<ValueType>::reset(const Vector &b)
    {
        b_norm = cusp::blas::nrm2(b);
        r_norm = std::numeric_limits<Real>::max();
        iteration_count_ = 0;
    ...
    bool monitor<ValueType>::finished(thrust::execution_policy<DerivedPolicy> &exec,
                                      const Vector &r)
    {
        ...
        if (converged())
        {
            if (verbose)
            {
                // std::cout << "Successfully converged after " << iteration_count() << " iterations." << std::endl;
                // std::cerr << "Successfully converged after " << iteration_count() << " iterations." << std::endl;
                // std::ofstream fp;
                // std::fp.open("gmres-cusp-out",std::ios::out);
                FILE *fp;
                fp = fopen("/home/liujie/gmres-lj/mixed-gmres-based-cusp/gmres-cusp-out2", "a+");
                fprintf(fp, "Successfully converged after %d iterations.\n", iteration_count());
                fclose(fp);
                // std::fp << "Successfully converged after " << iteration_count() << " iterations." << std::endl;
                // std::fp.close();
            }
            return true;
        }
        else if (iteration_count() >= iteration_limit())
        {
            if (verbose)
            {
                ...
                FILE *fp;
                fp = fopen("/home/liujie/gmres-lj/mixed-gmres-based-cusp/gmres-cusp-out2", "a+");
                fprintf(fp, "Failed to converge after %d iterations.\n", iteration_count());
                fclose(fp);
            }
            return true;
        }
    ...
    template <typename ValueType>
    template <typename Vector>
    bool monitor<ValueType>::finished(const Vector &r)
    {
    ...
    template <typename ValueType>
    typename monitor<ValueType>::Real
    monitor<ValueType>::immediate_rate(void)
    {
    ...
    template <typename ValueType>
    typename monitor<ValueType>::Real
    monitor<ValueType>::geometric_rate(void)
    {
        size_t num = residuals.size();
    ...
    template <typename ValueType>
    typename monitor<ValueType>::Real
    monitor<ValueType>::average_rate(void)
    {
        size_t num = residuals.size();
