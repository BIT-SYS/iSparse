                 ...
                 Preconditioner &M)
{
    ...
    float* xs_dev;
    cudaMalloc((void **)(&xs_dev),sizeof(float)*N);
    cudaMemcpy(xs_dev,xs,sizeof(float)*N,cudaMemcpyHostToDevice);
    double* xd_dev;
    cudaMalloc((void **)(&xd_dev),sizeof(double)*N);
    cudaMemcpy(xd_dev,&x[0],sizeof(double)*N,cudaMemcpyHostToDevice);

    double* y_cpu = (double*)malloc(N*sizeof(double));
    double* y_dev;
    cudaMalloc((void **)(&y_dev),sizeof(double)*N);

    float* xs_tmp=(float*)malloc(sizeof(float)*N);
    float* xs_tmp_dev;
    cudaMalloc((void **)(&xs_tmp_dev),sizeof(float)*N);
    double* xd_tmp_dev;
    cudaMalloc((void **)(&xd_tmp_dev),sizeof(double)*N);
    do
    {
        // compute initial residual and its norm
        // cusp::multiply(exec, A, x, w);                // V(0) = A*x
        if(monitor.iteration_count()<20){
            mixed_spmv(&AH, &AS, &AD, xs_dev, xd_dev, y_dev);
            cudaMemcpy(y_cpu,y_dev,sizeof(double)*N,cudaMemcpyDeviceToHost);
            for(unsigned int p=0;p<N;p++){
                w[p]=y_cpu[p];
            }
        }
        else{
            cusp::multiply(exec, A, x, w);  
        }
        ...
        do
        {
            ...
            //     y_cpu[p]=w[p];
            // }
            // cudaMemcpy(xs_tmp_dev,xs_tmp,sizeof(float)*N,cudaMemcpyHostToDevice);
            
            // cudaMemcpy(xd_tmp_dev,y_cpu,sizeof(double)*N,cudaMemcpyHostToDevice);
            // mixed_spmv(&AH, &AS, &AD, xs_tmp_dev, xd_dev, y_dev);
            // cudaMemcpy(y_cpu,y_dev,sizeof(double)*N,cudaMemcpyDeviceToHost);
            // for(unsigned int p=0;p<N;p++){
            //     w[p]=y_cpu[p];
            // }
