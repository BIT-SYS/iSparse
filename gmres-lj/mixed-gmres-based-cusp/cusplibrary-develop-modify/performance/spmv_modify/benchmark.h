...
const char * BENCHMARK_OUTPUT_FILE_NAME = "CUSPvector_time_SM8_1921mtx_V100.csv";
...
float time_spmv(TestMatrix& test_matrix, TestKernel test_spmv, double seconds = 3.0, size_t min_iterations = 1000, size_t max_iterations = 1000)
{
    typedef typename TestMatrix::index_type   IndexType; // ASSUME same as HostMatrix::index_type
    typedef typename TestMatrix::value_type   ValueType; // ASSUME same as HostMatrix::value_type
    typedef typename TestMatrix::memory_space MemorySpace;
...
template <typename TestMatrix, typename TestKernel>
float time_spmv_block(TestMatrix& test_matrix, size_t num_cols, TestKernel test_spmv, double seconds = 3.0, size_t min_iterations = 1000, size_t max_iterations = 1000)
{
    typedef typename TestMatrix::index_type   IndexType; // ASSUME same as HostMatrix::index_type
    typedef typename TestMatrix::value_type   ValueType; // ASSUME same as HostMatrix::value_type
    typedef typename TestMatrix::memory_space MemorySpace;
...
void test_coo(HostMatrix& host_matrix)
{
	...
	double millisec_transfer = transfer.seconds_elapsed() * 1000.0;
    
	//record results to file
    FILE * fid = fopen(BENCHMARK_OUTPUT_FILE_NAME, "a");
    fprintf(fid, "msec_transfer=%f ", millisec_transfer);
    fclose(fid);
