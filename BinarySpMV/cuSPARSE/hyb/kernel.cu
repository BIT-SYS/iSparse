...
int main(int argc, char *argv[])
{
	...
	while ((matrix_entry = readdir(matrix_dir_handle)) != NULL)
	{
		if (strcmp(matrix_entry->d_name, "..") != 0 && strcmp(matrix_entry->d_name, ".") != 0)
		{
			...
			printf("%s\n", matrixName);
            // if (strcmp(matrixName, ".DS_Store") == 0)
            //     continue;
...
int cudaSpmv(int *&csrRowIndexHostPtr, int *&csrColIndexHostPtr, float *&csrValHostPtr, float *&xHostPtr, float *&yHostPtr)
{
	...
	// FILE  *fresult = fopen("cusparse_hyb_time_float_V100.txt", "a+");
	// if (fresult == NULL) {printf("Create file failed.\n ");}
	// fprintf(fresult, "%s %.6f %.6f ", matrixName, transfer_time, var_transfer_time);

	cusparseStatus_t status_csr2hyb = CUSPARSE_STATUS_SUCCESS;  
	cusparseHybMat_t hybA;
	cusparseCreateHybMat(&hybA);
    double calculate_time = 0.0;
    double all_spmv_time[NUM_RUN] = {0};
	// convert csr to hyb
	status_csr2hyb = cusparseScsr2hyb(handle, M, N, descr, csrValGpuPtr, csrRowIndexGpuPtr, csrColIndexGpuPtr, hybA, 0, CUSPARSE_HYB_PARTITION_MAX);
	if (status_csr2hyb != CUSPARSE_STATUS_SUCCESS) {
		printf("cusparseScsr2hyb error, error_code = %d\n", status_csr2hyb);
		printf("%s N/A\n", matrixName);
		// fprintf(fresult, "calculation time N/A\n");
		// fclose(fresult);
	}
	else {
        // run one time for checking result
        checkcuda(cudaMemset(yGpuPtr, 0.0, M * sizeof(float)));
		checkcusparse( cusparseShybmv(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &done, descr, hybA, xGpuPtr, &dzero, yGpuPtr) );
		...
		// fprintf(fresult, "%f %.6f\n", calculate_time, var_calculate_time);
		// fclose(fresult);
