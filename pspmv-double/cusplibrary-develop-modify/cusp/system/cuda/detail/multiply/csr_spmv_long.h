                       ...
                       BinaryFunction2 reduce)
{
    ...
    __shared__ volatile ValueType sum[VECTORS_PER_BLOCK * THREADS_PER_VECTOR];  // padded to avoid reduction conditionals
    ...
    for(IndexType row = blockIdx.x; row < num_rows; row += gridDim.x)
    {
        ...
        const IndexType row_start = Ap[row + 0];
        ...
        for (IndexType i = row_start + threadIdx.x; i < row_end; i += blockDim.x)
