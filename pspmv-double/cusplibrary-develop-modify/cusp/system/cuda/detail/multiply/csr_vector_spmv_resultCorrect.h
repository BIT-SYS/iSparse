                       ...
                       BinaryFunction2 reduce)
{
    ...
    //__shared__ volatile ValueType sdata[VECTORS_PER_BLOCK * THREADS_PER_VECTOR + THREADS_PER_VECTOR / 2][8];  // padded to avoid reduction conditionals
     __shared__ ValueType sdata[VECTORS_PER_BLOCK * THREADS_PER_VECTOR + THREADS_PER_VECTOR / 2][MAX_FOLD_ROWS];  // padded to avoid reduction conditionals
    ...
    for(IndexType row = vector_id; row < num_rows; row += num_vectors)
    {
		...
		for (IndexType i = 0; i < MAX_FOLD_ROWS; i++) {
			sdata[threadIdx.x][i] = (ValueType)0.0;
		}
        ...
        if (THREADS_PER_VECTOR == 32 && row_end - row_start > 32)
        {
            ...
            if(jj >= row_start && jj < row_end) {
				...
				sdata[threadIdx.x][blockId] = reduce(sdata[threadIdx.x][blockId], combine(Ax[jj], x[colIdx]));
			}
        ...
        if (THREADS_PER_VECTOR > 16) {
			for (IndexType row = 0; row < rowIdx_end - rowIdx_start; row++) {
        	    temp = sdata[threadIdx.x + 16][row];
        	    sdata[threadIdx.x][row] = reduce(sdata[threadIdx.x][row], temp);
        	}
		}
        if (THREADS_PER_VECTOR >  8) {
			for (IndexType row = 0; row < rowIdx_end - rowIdx_start; row++) {
        	    temp = sdata[threadIdx.x +  8][row];
        	    sdata[threadIdx.x][row] = reduce(sdata[threadIdx.x][row], temp);
        	}
		}
        if (THREADS_PER_VECTOR >  4) {
			for (IndexType row = 0; row < rowIdx_end - rowIdx_start; row++) {
        	    temp = sdata[threadIdx.x +  4][row];
        	    sdata[threadIdx.x][row] = reduce(sdata[threadIdx.x][row], temp);
        	}
		}
        if (THREADS_PER_VECTOR >  2) {
			for (IndexType row = 0; row < rowIdx_end - rowIdx_start; row++) {
        	    temp = sdata[threadIdx.x +  2][row];
        	    sdata[threadIdx.x][row] = reduce(sdata[threadIdx.x][row], temp);
        	}
		}
        if (THREADS_PER_VECTOR >  1) {
			for (IndexType row = 0; row < rowIdx_end - rowIdx_start; row++) {
        	    temp = sdata[threadIdx.x +  1][row];
        	    sdata[threadIdx.x][row] = reduce(sdata[threadIdx.x][row], temp);
        	}
		}
		...
		for (IndexType row = 0; row < rowIdx_end - rowIdx_start; row++) {
        	...
        	if (thread_lane == 0)  {
        	   y[row + rowIdx_start] = ValueType(sdata[threadIdx.x][row]);
