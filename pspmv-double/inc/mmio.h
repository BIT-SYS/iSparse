...
typedef char MM_typecode[4];//定义一个长度为4的char数组的别名 [0]:表示是矩阵 [1]:表示是稀疏矩阵  [2]:非零元的类型 [3]:存储的类型，比如对称非对称等
...
int mm_read_mtx_crd_data(FILE *f, uint M, uint N, uint nz, uint I[], uint J[],
		double val[], MM_typecode matcode);
int mm_read_mtx_crd_entry(FILE *f, uint *I, uint *J, double *real, double *img,
			MM_typecode matcode);

int mm_read_unsymmetric_sparse(const char *fname, uint *M_, uint *N_, uint *nz_,
                double **val_, uint **I_, uint **J_);
