...
extern void axpy3(uint Am, double *a, double *b, double s);
...
extern void rotation3(uint Am, double *H, double *cs, double *sn, double *s, uint i);
...
extern void dot2(uint n, double *a, double *b, double *ret);

void gmres_mixed(SpM<double> *A, double *x, double *b, uint restart) //传入的都是cpu上的数据
{
    ...
    double *H; // H[restart+1][restart]  存的其实是H QR 分解后的结果
    ...
    cusp::monitor<double> monitor(monitor_b, 500, 1e-6, 0, true); // tolerance = 1e-6
    ...
    { // cpu 和kernel之间异步执行
        ...
        axpy3(A_dev.nrows, b_dev, r0_dev, -1.0); // r0=b-r0
        ...
        do
        {
            ...
            for (k = 0; k <= i; k++)
            {
                ...
                // printf("2.dot   %f\n", (H_cpu[0]));
                axpy3(A_dev.nrows, V_dev + A_dev.nrows * k, V0, -H_cpu[0]);
                // cudaDeviceSynchronize();
            }
            norm2(A_dev.nrows, V0, H + (i + 1) * restart + i);
            cudaMemcpy(H_cpu, H + (i + 1) * restart + i, sizeof(double), cudaMemcpyDeviceToHost);
            // cudaDeviceSynchronize();
            // printf("3.dot   %le\n", (H_cpu[0]));
            ...
            resid[0] = cusp::abs(H_cpu[0]); //相对误差
            // cout << "gmres:" << resid[0] << endl;
        ...
        for (j = 0; j <= i; j++)
        {
            ...
            axpy3(A_dev.nrows, V_dev + j * A_dev.nrows, x_dev, H_cpu[0]);
        }
