    ...
    for(IndexType row = vector_id; row < num_rows; row += num_vectors)
    {
        ...
        if (THREADS_PER_VECTOR == 32 && row_end - row_start > 32)
        {
            ...
            if(jj >= row_start && jj < row_end)
			{
				...
				// IndexType col = Aj[jj];
                // sum = reduce(sum, combine(Ax[jj], col));
				// sum += col;
			}
                       ...
                       BinaryFunction2 reduce)
{
	...
	cudaFuncSetAttribute(spmv_csr_vector_kernel<RowIterator, ColumnIterator, ValueIterator1, ValueIterator2, ValueIterator3,
                         UnaryFunction, BinaryFunction1, BinaryFunction2, VECTORS_PER_BLOCK, THREADS_PER_VECTOR>, 
