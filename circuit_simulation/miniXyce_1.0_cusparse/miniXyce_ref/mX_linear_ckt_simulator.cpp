...
int main(int argc, char* argv[])
{
    ...
    std::cout << ckt_netlist_filename << std::endl;
    char matrixName[256] ;
    char temp[256] = {0};
    ...
    cusparseCreateCsr(&matA, M, N, nnz,
                      thrust::raw_pointer_cast(&csrRowPtr[0]), 
                      thrust::raw_pointer_cast(&csrColIdx[0]), 
                      thrust::raw_pointer_cast(&csrVal[0]),
	...
	while (t < t_stop)
	{
        ...
        //         total_gmres_res += res;
        my_gmres(M, N, nnz, &matA, RHS, x, tol, res, k, x, iters, restarts);
        total_gmres_iters += iters;
        total_gmres_res += res;
    ...
    FILE *fresult = fopen("output_TPV2_double_V100_vectorCSR.txt", "a+");
 	if (fresult != NULL) {
