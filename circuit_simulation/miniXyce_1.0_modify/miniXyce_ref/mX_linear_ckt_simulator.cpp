...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Parameter_parsing done" << std::endl;
    std::cout << "ckt_netlist_filename " << ckt_netlist_filename << std::endl;

    char temp[256] = {0};
    strcpy(temp, ckt_netlist_filename.c_str());
    char *mtx_pure_name = strrchr(temp, '/');
    int len = strlen(mtx_pure_name);
    mtx_pure_name[len - 4] = '\0';
    strcpy(matrixName, mtx_pure_name+1);
    // strcpy(matrixName, ckt_netlist_filename.c_str());
    ...
    std::cout << "start readMtxToDAE" << std::endl;
    mX_linear_DAE *dae = readMtxToDAE(ckt_netlist_filename, p, pid, n, num_internal_nodes, num_voltage_sources, num_current_sources,
                                      num_resistors, num_capacitors, num_inductors);
    std::cout << "start readMtxToDAE done" << std::endl;
    ...
    std::cout << "num_my_rows = " << num_my_rows << ", num_my_nnz = " << num_my_nnz << std::endl;
    std::cout << "min_nnz = " << min_nnz << ", min_rows = " << min_rows << std::endl;
    ...
    // for (int i = start_row; i <= end_row; i++) x[i]=1;
    // x.resize(end_row-start_row+1);
    tstart = mX_timer();
    std::cout << "check initial condition" << std::endl;
    ...
    std::cout << "check initial condition done" << std::endl;
    tstart = mX_timer();
    ...
    std::cout << "start actual transient simulation" << std::endl;
    ...
    int_vector_device csrRowPtr, csrColIdx;
    double_vector_device csrVal;
    int M, N, nnz;
    matrix_to_three_vector(M, N, nnz, A, csrRowPtr, csrColIdx, csrVal);

    while (t < t_stop)
    {
        trans_steps++;
        std::cout << "trans_steps = " << trans_steps << std::endl;
        ...
        // print_vector(x);
        // std::cout << "call sparse_matrix_vector_product" << std::endl;

        sparse_matrix_vector_product(B, x, RHS);
        // std::cout << "call sparse_matrix_vector_product done" << std::endl;
        ...
        // std::cout << "call gmres" << std::endl;
        // gmres(A, RHS, x, tol, res, k, x, iters, restarts);
        // std::cout << "call gmres start" << std::endl;
        my_gmres(M, N, nnz, csrRowPtr, csrColIdx, 
                            csrVal, RHS, x, tol, res, k, x, iters, restarts);
        total_gmres_iters += iters;
        total_gmres_res += res;
        // std::cout << "total_gmres_iters: " << total_gmres_iters << ", total_gmres_res: " << total_gmres_res << std::endl;
        // std::cout << "call gmres done" << std::endl;
    ...
    FILE *fresult = fopen("output_TPV32_double_V100_vectorCSR.txt", "a+");
 	if (fresult != NULL) {
 		char ch=fgetc(fresult);
 		if (ch == EOF) {// file is empty 
 			fprintf(fresult, "Matrix M N nnz total_gmres_iters trans_steps transient_cal_time total_simulation_time\n");
 		}
 	}
 	else {
 		printf("open file failed\n");
 	}
    fprintf(fresult, "%s %d %d %d %d %d %lf %lf\n", matrixName, M, N, nnz, total_gmres_iters, trans_steps, tend-tstart, sim_end);
	fclose(fresult);
