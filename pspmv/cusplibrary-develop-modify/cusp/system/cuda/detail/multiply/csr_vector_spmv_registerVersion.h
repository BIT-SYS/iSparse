                       ...
                       BinaryFunction2 reduce)
{
    ...
    for(IndexType row = vector_id; row < num_rows; row += num_vectors)
    {
        ...
        if(thread_lane < 2)
            ptrs[vector_lane][thread_lane] = Ap[num_rows+1 + row + thread_lane];
        if(thread_lane >= 2 && thread_lane < 4)
            ptrs[vector_lane][thread_lane] = Ap[row + thread_lane - 2];
        ...
        const IndexType rowIdx_start = ptrs[vector_lane][2];                   //same as: row_start = Ap[row];
        const IndexType rowIdx_end   = ptrs[vector_lane][3];                   //same as: row_end   = Ap[row+1];
...
// 		}
		...
		sdata[0][threadIdx.x] = sum[0];
		sdata[1][threadIdx.x] = sum[1];
		sdata[2][threadIdx.x] = sum[2];
		sdata[3][threadIdx.x] = sum[3];
		sdata[4][threadIdx.x] = sum[4];
		sdata[5][threadIdx.x] = sum[5];
		sdata[6][threadIdx.x] = sum[6];
		sdata[7][threadIdx.x] = sum[7];

    	if (THREADS_PER_VECTOR > 16) {
    	    temp = sdata[0][threadIdx.x + 16];
    	    sdata[0][threadIdx.x] = sum[0] = reduce(sum[0], temp);
    	    temp = sdata[1][threadIdx.x + 16];
    	    sdata[1][threadIdx.x] = sum[1] = reduce(sum[1], temp);
    	    temp = sdata[2][threadIdx.x + 16];
    	    sdata[2][threadIdx.x] = sum[2] = reduce(sum[2], temp);
    	    temp = sdata[3][threadIdx.x + 16];
    	    sdata[3][threadIdx.x] = sum[3] = reduce(sum[3], temp);
    	    temp = sdata[4][threadIdx.x + 16];
    	    sdata[4][threadIdx.x] = sum[4] = reduce(sum[4], temp);
    	    temp = sdata[5][threadIdx.x + 16];
    	    sdata[5][threadIdx.x] = sum[5] = reduce(sum[5], temp);
    	    temp = sdata[6][threadIdx.x + 16];
    	    sdata[6][threadIdx.x] = sum[6] = reduce(sum[6], temp);
    	    temp = sdata[7][threadIdx.x + 16];
    	    sdata[7][threadIdx.x] = sum[7] = reduce(sum[7], temp);
    	}
    	if (THREADS_PER_VECTOR >  8) {
    	    ...
    	    sdata[3][threadIdx.x] = sum[3] = reduce(sum[3], temp);
    	    temp = sdata[4][threadIdx.x + 8];
    	    sdata[4][threadIdx.x] = sum[4] = reduce(sum[4], temp);
    	    temp = sdata[5][threadIdx.x + 8];
    	    sdata[5][threadIdx.x] = sum[5] = reduce(sum[5], temp);
    	    temp = sdata[6][threadIdx.x + 8];
    	    sdata[6][threadIdx.x] = sum[6] = reduce(sum[6], temp);
    	    temp = sdata[7][threadIdx.x + 8];
    	    sdata[7][threadIdx.x] = sum[7] = reduce(sum[7], temp);
    	}
    	if (THREADS_PER_VECTOR >  4) {
    	    ...
    	    sdata[3][threadIdx.x] = sum[3] = reduce(sum[3], temp);
    	    temp = sdata[4][threadIdx.x + 4];
    	    sdata[4][threadIdx.x] = sum[4] = reduce(sum[4], temp);
    	    temp = sdata[5][threadIdx.x + 4];
    	    sdata[5][threadIdx.x] = sum[5] = reduce(sum[5], temp);
    	    temp = sdata[6][threadIdx.x + 4];
    	    sdata[6][threadIdx.x] = sum[6] = reduce(sum[6], temp);
    	    temp = sdata[7][threadIdx.x + 4];
    	    sdata[7][threadIdx.x] = sum[7] = reduce(sum[7], temp);
    	}
	   	if (THREADS_PER_VECTOR >  2) {
    	    ...
    	    sdata[3][threadIdx.x] = sum[3] = reduce(sum[3], temp);
    	    temp = sdata[4][threadIdx.x + 2];
    	    sdata[4][threadIdx.x] = sum[4] = reduce(sum[4], temp);
    	    temp = sdata[5][threadIdx.x + 2];
    	    sdata[5][threadIdx.x] = sum[5] = reduce(sum[5], temp);
    	    temp = sdata[6][threadIdx.x + 2];
    	    sdata[6][threadIdx.x] = sum[6] = reduce(sum[6], temp);
    	    temp = sdata[7][threadIdx.x + 2];
    	    sdata[7][threadIdx.x] = sum[7] = reduce(sum[7], temp);
	   	}
	   	if (THREADS_PER_VECTOR >  1) {
    	    ...
    	    sdata[3][threadIdx.x] = sum[3] = reduce(sum[3], temp);
    	    temp = sdata[4][threadIdx.x + 1];
    	    sdata[4][threadIdx.x] = sum[4] = reduce(sum[4], temp);
    	    temp = sdata[5][threadIdx.x + 1];
    	    sdata[5][threadIdx.x] = sum[5] = reduce(sum[5], temp);
    	    temp = sdata[6][threadIdx.x + 1];
    	    sdata[6][threadIdx.x] = sum[6] = reduce(sum[6], temp);
    	    temp = sdata[7][threadIdx.x + 1];
    	    sdata[7][threadIdx.x] = sum[7] = reduce(sum[7], temp);
	   	}
