...
// 		}
		...
		ValueType temp;
 		...
 		for (IndexType row = 0; row < rowIdx_end - rowIdx_start; row++) {
			sdata[row][threadIdx.x] = sum[row];

         	if (THREADS_PER_VECTOR > 16) {
         	    temp = sdata[row][threadIdx.x + 16];
         	    sdata[row][threadIdx.x] = sum[row] = reduce(sum[row], temp);
         	}
         	if (THREADS_PER_VECTOR >  8) {
         	    temp = sdata[row][threadIdx.x + 8];
         	    sdata[row][threadIdx.x] = sum[row] = reduce(sum[row], temp);
         	}
         	if (THREADS_PER_VECTOR >  4) {
         	    temp = sdata[row][threadIdx.x + 4];
         	    sdata[row][threadIdx.x] = sum[row] = reduce(sum[row], temp);
         	}
        	if (THREADS_PER_VECTOR >  2) {
        	    temp = sdata[row][threadIdx.x + 2];
        	    sdata[row][threadIdx.x] = sum[row] = reduce(sum[row], temp);
        	}
        	if (THREADS_PER_VECTOR >  1) {
        	    temp = sdata[row][threadIdx.x + 1];
        	    sdata[row][threadIdx.x] = sum[row] = reduce(sum[row], temp);
        	}
