...
{ 
    ...
    for(IndexType row = thread_id; row < num_rows; row += grid_size)
    {
        ...
        const IndexType s_rowPtr = Ap[num_rows+1 + row];
        const IndexType e_rowPtr = Ap[num_rows+1 + row+1];
		...
		// for (IndexType i = 0; i < e_rowIdx - s_rowIdx; i++) sum[i * blockDim.x + threadIdx.x] = 0;
    }
}
